...
const fields = require('../config/builder.fields.js');
const { handleIDExtractorReaction } = require('./id-fetch.system');
  ...
  if (!message.embeds.length) {
    setTimeout(async () => {
      try {
        ...
        sendError('Error fetching message after delay:', error);
      }
    }, 2000);
    return;
  }
...
async function processInventoryEmbed(message) {
  ...
  if (embed.title.includes('<:LU_Inventory:') && embed.title.includes("'s Inventory")) {
    try {
      ...
      sendError('Failed to react to inventory:', error);
    }
  } else if (embed.description && /^\d+\.\s+\w+\s+\(ID:\s*`\d+`\)/m.test(embed.description)) {
    try {
      await message.react('🆔');
    } catch (error) {
      sendError('Failed to react to team:', error);
    }
  }
}
...
async function handleCommandBuilderReaction(reaction, user) {
  ...
  try {
    ...
    sendError('Failed to send generator message:', error);
  }
}
...
async function handleNameSelect(interaction) {
  ...
  try {
    ...
    sendError('Error in handleNameSelect:', error);
    return true;
  }
}
...
async function handleSelectFieldValue(interaction) {
  ...
  try {
    ...
    sendError('Error updating main message:', error);
  }
...
function startInventoryWatcher(userId, inventoryMessage) {
  const checkInterval = setInterval(async () => {
    ...
    try {
      ...
      sendError('Error in inventory watcher:', error);
      clearInterval(checkInterval);
    }
  }, 2000);
